...
export const getPrice  = async (tokenInContract , tokenOutContract , pairAddress) => {
  ...
  const formatbalanceToken0InPool  = await ethers.utils.formatUnits(balanceToken0InPool , decimal0);
  const formatbalanceToken1InPool  = await ethers.utils.formatUnits(balanceToken1InPool , decimal1);
  const busdBalance = decimal0 === 18 ? formatbalanceToken0InPool : formatbalanceToken1InPool
  const hectaBalance = decimal1 === 9 ? formatbalanceToken1InPool : formatbalanceToken0InPool
  console.log(`balance  BUSD pool in AMM : ${busdBalance}`);
  console.log(`balance  HECTA pool in AMM : ${hectaBalance}`);
  let price = 0 
  if(decimal1 === 9) {
    price = parseFloat(formatbalanceToken0InPool)/ parseFloat(formatbalanceToken1InPool)
  }else {
    price = parseFloat(formatbalanceToken1InPool)/ parseFloat(formatbalanceToken0InPool)
  }
...
export const buyAction = async ({ recipient, privateKey, tokenIn, tokenOut, amountSwapToken0 }) => {
  try {
    ...
    const {price} = await getPrice(tokenInContract , tokenOutContract , pairAddress)
    ...
    if (parseInt(configData.SLIPPAGE) !== 0) {
      ...
      amountOutMin = amounts[1].sub(amounts[1].mul(`${configData.SLIPPAGE}`).div(`100`))
      
    }
    // check if approved 
   
    const allowanceToken0 = await tokenInContract.allowance(account.address, tokenIn);
    const allowanceToken1 = await tokenOutContract.allowance(account.address, tokenOut);
  
    // approve router contract call to busd 
    if (!alreadyApprovedToken(allowanceToken0)  ) {
      const approveTx = await tokenInContract.approve(tokenIn, "1000000000000000000000000", {
    ...
    if (!alreadyApprovedToken(allowanceToken1) ) {
      const approveTx = await tokenOutContract.approve(tokenOut, "1000000000000000000000000", {
        gasPrice: ethers.utils.parseUnits(`${100}`, 'gwei').toString(),
        gasLimit: 1000000
      });
    }

    console.log("proccecing ...")
    // call router smart contract to buy token
    const tx = await routerContract.swapExactTokensForTokens( //uncomment here if you want to buy token
      amountIn,
      amountOutMin,
      [tokenIn, tokenOut],
      recipient,
    ...
    const {price : newPrice, busdBalance : newBusdBalance  , hectaBalance : newHectaBalance} = await getPrice(tokenInContract , tokenOutContract , pairAddress)
